...
Sending a Transaction
Creating the keypairs
To put the HodlVault.cash contract to use in a TypeScript application, we have to use the CashScript SDK in combination with a BCH library such as Libauth, Mainnetjs or BCHJS. These libraries are used to generate public/private keys for the contract participants. In this example we'll use Libauth to generate the keys alicePriv, alicePub, oracle & oraclePub. Then we can use these keys to create the smart contract.
